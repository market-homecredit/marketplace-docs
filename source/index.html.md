...
Каталог обязательно должен иметь XML-заголовок, который начинается с первой строки, с нулевого элемента, в заголовке присутствуют атрибуты версии и используемая кодировка. Допустимая кодировка - windows-1251. Пример заголовка:
...
**Резервирование** — после того, как Покупатель заполнил заявку, выбрал способ доставки и его заявка была одобрена системой скоринга банка, партнеру приходит запрос на резервирование товара до момента подписания кредитного договора клиентом.
...
#Методы API маркетплейс 

## POST /partner/offers/

**Обновление предложения партнера**

Метод позволяет обратиться к API маркетплейса (https://api.market.homecredit.ru) и в случае изменения товарной позиции, партнер отправляет соответствующий запрос на обновление информации о предложении..

<aside class="notice">
Для получения данного функционала необходимо получить токен от сотрудника маркетплейс
который вставляем в запрос в виде заголовка x-application(см. пример запроса)
</aside>
 **Входные параметры**
`POST https://api.market.homecredit.ru/partner/offers/`


> Пример запроса POST https://api.market.homecredit.ru/partner/offers/:

```shell
curl -X POST \
  https://api.market.homecredit.ru/partner/offers/ \
  -H 'Cache-Control: no-cache' \
  -H 'Content-Type: application/json' \
  -H 'x-application:  a1a2c3aa-e11b-1234-b15c-ed6ad140d12m' \
  -d '{
  "offers": [
    {
      "offerId": "71367685",
      ...
      "price": 78000.00,
      "available": true,
      "url": "https://www.example.com/cat/detail/71367685/",
      "pickup": true,
      "points": [
        "A343",
        "A264"
...
|---------------------------------|----------------|--------------------------------------------------------------------------------------|
| `offerId`<br>  string | Обязательно | Идентификатор товара из YML-каталога. |
| `regionIds`<br> Array of string | Обязательно | Список идентификаторов регионов системы Маркетплейс, в которых произошли изменения. |
...
| `points`<br>   Array of string |   Необязательно | Список пунктов самовывоза, описывает доступные пункты самовывоза для получения данного предложения. |
...
## GET /credit_request/:orderId/status
 **Входные параметры**
`GET https://api.market.homecredit.ru/credit_request/:orderId/status`

> Пример запроса GET https://api.market.homecredit.ru/credit_request/:orderId/status:

```shell
curl -X GET \
 https://api.market.homecredit.ru/credit_request/12345600002212/status \
 -H 'Content-Type: application/json' \
 -H 'cache-control: no-cache' \
 -H 'x-application: a1a2c3aa-e11b-1234-b15c-ed6ad140d12m'
```

> Пример ответов в JSON:

```json
{
  "data": {
    "orderId": "12345600002212",
    "status": "signed",
    "agreementId": "1234818047"
  },
  "status": true
}
```
...
**Получение актуального статуса кредитной заявки и номера кредитного договора**

Метод позволяет обратиться к API маркетплейса (https://api.market.homecredit.ru) и получить актуальный статус по заявкам, а так же номер кредитного договора клиента.
