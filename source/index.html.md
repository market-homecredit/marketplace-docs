...
<aside class="notice">Тег &lt;name&gt; содержит в себе название товара, которое будет выведено в названии карточки товара на платформе Marketplace, и в соответствии с требованиями отдела контента, должно иметь определенную структуру.</aside>
<br>
...
<aside class="notice">
Файл .csv с регионами РФ, в которых работает Маркетплейс
</aside>
...
<aside class="success">Презентация с описанием кредитного лимита и корзины.</aside>
[Посмотреть](korzina_limit.pdf)
...
Метод CHECK означает поступающий партнеру запрос на проверку наличия одного, или нескольких товаров в регионе и вызывается в двух случаях:
<ul>
  <li> когда Покупатель добавляет товар в корзину</li>
  <li> в момент когда Покупатель переходит из корзины к оформлению заказа</li>
</ul>

В ответ на запрос партнер должен уведомить о возможности зарезервировать товар, а также прислать условия доставки и самовывоза. Расчет параметров получения товара клиентом должен производиться на основе доступных товаров. (Например, если доступно только 3 из 5 переданных в запросе товаров, стоимость доставки должна быть только для этих трех товаров).
...
| `productCode`<br> string | Обязательно    | Код кредитного продукта (срок рассрочки).<br> Примеры:<ul><li>0-0-3</li><li>0-0-6</li><li>0-0-10</li><li>0-0-12</li><li>0-0-18</li><li>0-0-24</li><li>0-0-36</li></ul> |

<aside class="warning">
Обратите внимание, что клиент может выбрать разные сроки рассрочки для каждого оффера!
</aside>
...
### Параметры ответа
`HTTP status code 200 OK — Оба товара зарезервированы(ответ на метод reserve).`
...
|-------------------------------------|----------------------------------------------------------------------|----------------------------------------------------------------------------------|
| Заказ оплачен<br>(PAYED) | Клиент подтвердил оплату заказа, и деньги были переведны на счет партнера. | Партнер может выдать товар на пункте самовывоза, либо начинает процесс доставки. |
| Кредитный договор подписан (До перехода на процесс кредитных лимитов)<br>(SIGNED) | Покупатель подписал кредитный договор и ожидает получение товара. | Партнер может выдать товар на пункте самовывоза, либо начинает процесс доставки. |
| Заказ отменен<br>(CANCELLED) | Отказ клиента. Также можно интерпретировать как отмена бронирования. | Партнер отменяет заказ. |
...
`POST https://example.com/order/:orderId/status со статусами SIGNED и CANCELLED`
...
<aside class="warning">
Описание этих методов приводится ТОЛЬКО ДЛЯ ОЗНАКОМЛЕНИЯ. Текущая реализация все еще поддерживает их, но вскоре будет отключена. Настоятельно рекомендуется использование методов с поддержкой продуктовой корзины.
</aside>
