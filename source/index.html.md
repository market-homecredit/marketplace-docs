...
| `categories`  <br> Обязательный| Состоит из списка элементов category, каждый из которых описывает категорию для товаров в каталоге. |
| `offers`<br> Обязательный|  Состоит из списка элементов offer,  каждый из которых описывает товар в каталоге. |
...
| Атрибут| Описание |
|-------------------------|--------------------------------------------------------------------------------------|
| `id`<br> Обязательный | Идентификатор товарного предложения. В XML является атрибутом для offer. |
...
| `credits`<br>Обязательный, если был заявлен атрибут credit со значением list | Включает список элементов программ рассрочек <credit> типа credit, описывает доступные кредитные программы для каждого из товаров |
...
| `delivery-options` <br> Необязательный| Параметр содержит список сроков и стоимости доставки для данного товара. Родительский элемент для option.|
| `categoryId` <br> Обязательный| Идентификатор категории товара для данного предложения|
| `name` <br> Обязательный| Идентификатор категории товара для данного предложения| 
Название товара, не более 120 символов. Может содержать в себе название товара, производитель или бренд, модель, а также любые другие важные элементы.|
