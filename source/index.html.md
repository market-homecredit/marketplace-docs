...
Партнер предоставляет каталог товаров в виде выгрузки прайс-листа в формате XML.

Базовый сценарий предполагает, что на каждый регион необходимо задать свой каталог.  
Возможен вариант, когда каталог товаров един для всех регионов, данный сценарий необходимо предварительно согласовать с сотрудниками интеграции.

Согласно <a href='https://market.homecredit.ru/pages/usloviia-obespecheniia-urovnia-servisa'>Условиям обсепечения уровня сервиса</a>, партнерская выгрузка должна обновляться не менее пяти раз в день, для сохранения актуальной информации о доступности ассортимента.
...
Выгруженный каталог может содержать в себе любые другие параметры и элементы, кроме приведенных ниже, например, всю структуру YML (стандарт, разработанный Яндексом для принятия и размещения информации в базе данных Яндекс.Маркета), что не будет вызывать ошибок обработки каталога со стороны Маркетплейса – мы игнорируем при парсинге теги, которые не прописаны в документации ниже.

## Каталог в формате XML

Каталог обязательно должен иметь XML-заголовок, который начинается с первой строки, с нулевого элемента, в заголовке присутствуют атрибуты версии и используемая кодировка. Во избежание ошибок парсинга, допускается использование только кодировки UTF-8. 
В начале файла обязательно должен быть указан заголовок: <?xml version="1.0" encoding="UTF-8"?>
    ...
    <shop>
        ...
        <offers>
            ...
            <offer id="262" available="true" credit="list">
                ...
                <vendor>Apple</vendor>
                <vendorCode>MTFR2RU/A</vendorCode>
...
| Элемент| Описание |
|-------------------------|--------------------------------------------------------------------------------------|
| `name`<br> Обязательный |Короткое описание магазина партнера. |
...
| Атрибут| Описание |
|-------------------------|--------------------------------------------------------------------------------------|
| `id` <br> Необязательный | Идентификатор категорий. |
...
| Элементы| Описание |
|-------------------------|--------------------------------------------------------------------------------------|
| `url`<br>  Обязательный | URL страницы товара на сайте партнера. |
| `price`<br> Обязательный | Цена товара на основе которой будет рассчитан ежемесячный платеж для рассрочки. |
| `oldprice`<br> Необязательный | Старая цена товара, которая должна быть выше текущей. При наличии данного тега у предложения, для него будет посчитана скидка и выведена на фронт. |
| `credits`<br>Обязательный, если был заявлен атрибут credit со значением list | Включает список элементов тарифов [credit](#credit), описывает доступные тарифы для каждого из товаров. Учитывается на основном сайте, и не учитывается для HomeShop. |
| `categoryId` <br> Необязательный | Идентификатор категории товара для данного предложения|
| `name` <br> Обязательный, если не указаны model и vendor | Название товара, не более 120 символов. Может содержать в себе название товара, производитель или бренд, модель, а также любые другие важные элементы. |
| `typePrefix` <br>  Необязательный |  Тип/категория товара. |
| `vendor` <br>  Необязательный | Имя производителя или бренда для товара. |
| `vendorCode` <br>  Обязательный | Артикул производителя. |
...
| `pickup`<br> Необязательный |  Обозначает доступность в пунктах самовывоза. Возможные значения:<ul><li>true — товар доступен в пунктах самовывоза. Дополнительно необходимо указать список пунктов самовывоза в параметре  points;</li><li>false — товар недоступен в пунктах самовывоза.</li></ul>  |
| `delivery` <br> Необязательный | Обозначает возможность доставки. Возможные значения:<ul><li>true — товар доступен для доставки;</li><li>false — доставка товара не предусмотрена.</li></ul>  |
...
<aside class="warning">На Маркетплейсе определен диапазон цен, в рамках которого в каталог попадают партнерские предложения. Для POS-процесса нижняя граница составляет 6000 р. (с учетом партнерской скидки), а верхняя – 180000 р. (с учетом партнерской скидки). Для ШК-процесса диапазон составляет от 1000 р. до 200000 р. (с учетом партнерской скидки). </aside>
...
<aside class="warning">
Обратите внимание, что объект "deliveryOptions" (варианты доставки) должен быть один для всех офферов!
...
| `productCode`<br> string | Необязательно | Код тарифа.<br> Примеры:<ul><li>0-0-0</li><li>0-0-1</li><li>0-0-2</li><li>0-0-3</li><li>0-0-6</li><li>0-0-10</li><li>0-0-12</li><li>0-0-18</li><li>0-0-24</li><li>0-0-36</li></ul> |
...
Описанные в этом разделе особенности обновлений API соответствуют фазам внедрения на Маркетплейс новой функциональности, для поддержки которой партнерам необходимо поэтапно внести изменения в партнерское API со своей стороны. Данная информация приводится для партнеров, уже имеющих интеграцию с сервисом.

Схемы и описание устаревших методов API доступны в нашем <a href=https://docs.homecred.it/old_api.html>архиве</a> **только для ознакомления**.

## 1.0 - Поддержка оформления заказа на кредитный договор
...
Актуальная версия API, в которой поддерживается полный функционал корзины, а так же полный набор методов, необходимых для работы с процессом кредитного лимита.
...
С переходом на эту версию для партнеров, уже имеющих интеграцию, будет доступен новый процесс обработки заказов с оплатой клиентом заказа картой, с использованием внутреннего эквайринга. Подробнее о нововведениях можно посмотреть в [этой презентации](korzina_limit.pdf).

<aside class="notice">На стороне партнера в этой версии должна быть поддержка как нового **ШК-процесса**, так и старого **POS-процесса** оформления заказа, т.к. по старому процессу будет происходить оплата заказов с офомлением кредитного договора в стороннем банке.

* POS-процесс – подписание клиентом кредитного договора (передается статус подписания договора **SIGNED**). 
* ШК-процесс – покупка с оплатой заказа картой, или кредитного лимита VKPay (передается статус оплаты заказа **PAID**).
</aside>
<br>